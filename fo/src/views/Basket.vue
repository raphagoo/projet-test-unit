<template>
    <v-container fluid class="fill-height grey lighten-5">
        ...
        <v-row dense no-gutters>
            <v-col class="pa-1" cols="3" v-for="product in basket.products" :key="product._id">
                <v-card class="mx-auto cart" outlined>
                    <v-list-item three-line>
                        ...
                        <v-list-item-avatar tile size="80"><v-img :src="product.image"></v-img></v-list-item-avatar>
                    </v-list-item>
                    <v-card-actions>
                        <v-btn v-if="product.quantity > 1" outlined rounded @click="removeOneProduct(product)" color="error">Retirer 1</v-btn>
                        <v-btn class="remove" outlined rounded @click="removeProduct(product)" color="error">Supprimer</v-btn>
                    </v-card-actions>
                </v-card>
            </v-col>
        </v-row>
        <div v-if="basket.products.length > 0">
            <stripe-checkout
                ...
                @loading="v => loading = v"
            />
            <v-btn class="ma-3 cyan addBdd"  @click="submit">SAVE !</v-btn>
            ...
            <v-row dense>
                <v-col cols="4" v-for="product in matchingProducts" :key="product.id">
                    <v-card class="mx-auto" outlined>
                        ...
                        <v-card-actions>
                            <v-btn  @click="addBasket()" outlined rounded text><v-icon>add</v-icon>Add to basket</v-btn>
                        </v-card-actions>
                    </v-card>
                </v-col>
            </v-row>
...
export default {
    ...
    methods: {
        ...mapActions('basket', {
            ...
            remove: 'remove',
            addBasketBDD: 'addBasketBDD'
        }),
        submit () {
            // You will be redirected to Stripe's secure checkout page
           
            this.addBasketBDD(this.$store.state.basket.productsId)
             console.log(this.$store.state.basket.productsId)
        },
